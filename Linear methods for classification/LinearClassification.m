function [model]=LinearClassification(x,y,standardize,type,options={})
  ...
  # - univariate non-parametric logistic regression (using splines)
  ...
  # univariate non-parametric logistic regression (using splines)
    case 'smoothing spline'
      ...
      addpath('../Basis expansions and regularization')
      options={'lambda',num2str(lambda)};
      splines=splines1D(x,type,options);

      beta_hat=zeros(size(splines.omega,1),1);
      p=logit(splines.h,beta_hat);
      w=eye(size(splines.h,1));

      spline_loglikelihood=@(beta_hat)(loglikelihood(splines.h,Y(:,2),beta_hat)-0.5*sum(splines.lambda.*splines.omega*(beta_hat.^2)));
      ...
      while run<3
        ...
        beta_hat=beta_old-pinv(hessian)*derivative;
        #beta_hat=pinv(splines.h'*w*splines.h+splines.lambda.*splines.omega)*splines.h'*w*(splines.h*beta_old+pinv(w)*(Y(:,2)-p));

        p=logit(splines.h,beta_hat);
        alpha=1;
        l=spline_loglikelihood(beta_hat);
        l_old=spline_loglikelihood(beta_old);
        l_diff=l-l_old;
        while l_diff<0
          alpha=alpha/2;
          beta_hat=beta_old-alpha*pinv(hessian)*derivative;
          l=spline_loglikelihood(beta_hat);
          l_old=spline_loglikelihood(beta_old);
          l_diff=l-l_old;
        end
        ...
        l_old=spline_loglikelihood(beta_old);
        p_old=logit(splines.h,beta_old);

        perf_old=mean(Y(:,2)==(G((p_old>threshold)+1)));
        perf_new=mean(Y(:,2)==(G((p>threshold)+1)));
